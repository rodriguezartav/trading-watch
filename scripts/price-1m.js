...
async function Run(){
    ...
    while(index<stocks.length){
        ...
        await knex.table("prices").insert({ stock_id_created_at: `${stock.id}_${createdAt}`, created_at: createdAt ,stock_id: stock.id, price: price.latestTrade.p }).onConflict("stock_id_created_at").merge()
        index++;
    }
